...
if($_POST)
{
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		if(mysqli_query($db,"INSERT INTO chatting(user, message, ip_address) value('$username','$message','$user_ip')"))
		{
		...
		mysqli_query($db,"DELETE FROM chatting WHERE id NOT IN (SELECT * FROM (SELECT id FROM chatting ORDER BY id DESC LIMIT 0, 50) as sb)");
	}
	elseif($_POST["fetch"]==1)
	{
		$results = mysqli_query($db,"SELECT user, message, date_time FROM (select * from chatting ORDER BY id DESC LIMIT 10) chatting ORDER BY chatting.id ASC");
